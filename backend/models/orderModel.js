...
module.exports = {
    ...
    getOrders: async ( res, buyer_id, seller_id  ) => {
        try {
            ...
            item.buyers_limit, item.num_of_buyers, item.title, item.image, item.introduction, item.cost, item.tag, item.item_location, DATE_FORMAT(item.created_at, "%Y-%m-%d %H:%i:%s") AS created_at, user.id, user.name, user.phone, user.image, user.rating \
            FROM order_table \
            ${userCondition} \
